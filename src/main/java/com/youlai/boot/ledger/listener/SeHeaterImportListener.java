 ...
 */
@Slf4j
public class SeHeaterImportListener extends AnalysisEventListener<SeHeaterExportDto> {
     ...
     */
    public SeHeaterImportListener() {
        this.seHeaterService = SpringUtil.getBean(SeHeaterService.class);
        this.seHeaterConverter = SpringUtil.getBean(SeHeaterConverter.class);
     ...
     * 2. 数据持久化；
     *
     * @param seHeaterExportDto 一行数据，类似于 {@link AnalysisContext#readRowHolder()}
     */
    @Override
    public void invoke(SeHeaterExportDto seHeaterExportDto, AnalysisContext analysisContext) {
        log.info("解析到一条用户数据:{}", JSONUtil.toJsonStr(seHeaterExportDto));
        ...
        String pressureTag = seHeaterExportDto.getSeHeaterEquipCode();
        if (StrUtil.isBlank(pressureTag)) {
            errorMsg += "编号为空；";
            validation = false;
        } else {
            long count = seHeaterService.count(new LambdaQueryWrapper<SeHeater>().eq(SeHeater::getSeHeaterEquipCode, pressureTag));
            if (count > 0) {
                errorMsg += "编号已存在；";
                validation = false;
            }
