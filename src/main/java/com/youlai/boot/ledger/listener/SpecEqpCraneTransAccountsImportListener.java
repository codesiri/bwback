 ...
 */
@Slf4j
public class SpecEqpCraneTransAccountsImportListener extends AnalysisEventListener<SpecEqpCraneTransAccountsExportDto> {
     ...
     */
    public SpecEqpCraneTransAccountsImportListener() {
        this.specEqpCraneTransAccountService = SpringUtil.getBean(SpecEqpCraneTransAccountService.class);
        this.specEqpCraneTransAccountConverter = SpringUtil.getBean(SpecEqpCraneTransAccountConverter.class);
     ...
     * 2. 数据持久化；
     *
     * @param specEqpCraneTransAccountsExportDto 一行数据，类似于 {@link AnalysisContext#readRowHolder()}
     */
    @Override
    public void invoke(SpecEqpCraneTransAccountsExportDto specEqpCraneTransAccountsExportDto, AnalysisContext analysisContext) {
        log.info("解析到一条用户数据:{}", JSONUtil.toJsonStr(specEqpCraneTransAccountsExportDto));
        ...
        String pressureTag = specEqpCraneTransAccountsExportDto.getSpecEqpCtName();
        if (StrUtil.isBlank(pressureTag)) {
            errorMsg += "位号为空；";
            validation = false;
        } else {
            long count = specEqpCraneTransAccountService.count(new LambdaQueryWrapper<SpecEqpCraneTransAccount>().eq(SpecEqpCraneTransAccount::getSpecEqpCtName, pressureTag));
            if (count > 0) {
                errorMsg += "位号已存在；";
                validation = false;
            }
        ...
        if (validation) {
            // 校验通过，持久化至数据库
            SpecEqpCraneTransAccount entity = specEqpCraneTransAccountConverter.toEntity(specEqpCraneTransAccountsExportDto);
            ...
            boolean saveResult = specEqpCraneTransAccountService.save(entity);
            if (saveResult) {
